...
exports.getCount = function(req, res) {
  Transaction.find().count(function(err, count) {
    if (err)
...
exports.limitedList = function(req, res) {
  ...
  Transaction.find().sort('-updated').skip((page - 1) * 8).limit(8)
...
exports.filterCarts = function(req, res) {
  ...
  Transaction.find({
    'updated': { $gt: date_start, $lt: date_end }
  }).sort('-updated').skip((page - 1) * 8).limit(8)
    .populate('user', 'username firstName lastName')
    ...
    if (err) {
      ...
      Transaction.find({
        ...
        if (err)
          ...
          res.json({'carts': carts, 'count': count});
        }
      });
    }
  });
